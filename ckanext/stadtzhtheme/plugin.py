...
RENKU_PY_URL = "[![renku](https://renkulab.io/renku-badge.svg)](https://renkulab.io/p/opendatazurich/starter-code/sessions/{renku_session_id}/start?PACKAGE_ID={package_id}&RESOURCE_ID={resource_id})"
RENKU_R_URL = "[![renku](https://renkulab.io/renku-badge.svg)](https://renkulab.io/p/opendatazurich/starter-code/sessions/{renku_session_id_r}/start?PACKAGE_ID={package_id})"
SQL_WORKBENCH_URL = "[![SQL](https://img.shields.io/badge/SQL-grey?style=flat&logo=DuckDB&logoSize=auto&labelColor=grey&color=grey&link=https%3A%2F%2Fsql-workbench.com)](https://sql-workbench.com/#queries=v0,%20%20-This-is-autogenerated-code-to-analyze-(meta)-data-from-OpenDataZurich,%20%20-All-information-refers-to-the-data-set-that-can-be-found-here%3A-{frontend_url_for_sql}%2Fdataset%2F{package_id},%20%20-get-metadata,CREATE-TABLE-metadata-AS-SELECT-*-FROM-read_json(%22{frontend_url_for_sql}%2Fapi%2Faction%2Fpackage_show%3Fid%3D{package_id}%22)~,%20%20-use-this-to-display-metadata,%20%20SELECT-result.title%2C-result.timeRange%2C-result.notes%2C-result.sszBemerkungen-FROM-metadata~,%20%20-get-data,CREATE-TABLE-ogdszh-AS-SELECT-*-FROM-read_{file_format}(%22https%3A%2F%2Fcors.sqlqry.run%2F%3Furl%3D{download_url}%22)~,%20%20-show-the-first-5-rows-of-the-data,SELECT-*-FROM-ogdszh-LIMIT-5~)"
DOWNLOAD_JUPYTER_URL = "[Jupyter-Notebook](https://github.com/opendatazurich/starter-code/blob/main/02_python/{package_id}_{resource_id}.ipynb)"
DOWNLOAD_R_URL = "[R-Markdown](https://github.com/opendatazurich/starter-code/blob/main/01_r-markdown/{package_id}_{resource_id}.Rmd)"

RENKU_MARKDOWN = f"""Analysiere diesen Datensatz direkt online – mit **Renku** in Python {RENKU_PY_URL} oder R {RENKU_R_URL}. Dieses **{DOWNLOAD_JUPYTER_URL}** sowie das **{DOWNLOAD_R_URL}** ist mit allen weiteren Skripts in diesem [**Github-Repo**](https://opendatazurich.github.io/starter-code/) zu finden."""
RENKU_WITH_SQL_MARKDOWN = f"""Analysiere diesen Datensatz direkt online – mit **Renku** in Python {RENKU_PY_URL} oder R {RENKU_R_URL}, oder führe **SQL-Abfragen** in der **SQL Workbench** {SQL_WORKBENCH_URL} aus. Dieses **{DOWNLOAD_JUPYTER_URL}** sowie das **{DOWNLOAD_R_URL}** ist mit allen weiteren Skripts in diesem [**Github-Repo**](https://opendatazurich.github.io/starter-code/) zu finden."""
...
class StadtzhThemePlugin(
    ...
    def _set_markdown_snippet_text(self, resource):
        ...
        download_url_for_sql = resource.get("url").replace("-","%20")
        frontend_url_for_sql = tk.config.get("ckanext.stadtzhtheme.frontend_url", "").replace("-","%20")
        ...
        if resource.get("url").lower().endswith("geojson_link"):
            ...
            resource["markdown_snippet"] = RENKU_WITH_SQL_MARKDOWN.format(
                ...
                resource_id=resource_id,
                file_format=file_format,
                download_url=download_url_for_sql,
